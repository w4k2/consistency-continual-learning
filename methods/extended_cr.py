...
class ExtendedConsistencyPlugin(SupervisedPlugin):
    ...
    def __init__(self, regularisation, mem_size: int = 200, alpha=1.0, beta=1.0, T=5, *args, **kwargs):
...
class ExtendedConsistencyRegularistaion(SupervisedTemplate):
    def __init__(self, model, optimizer, criterion,
        ...
        plugin = ExtendedConsistencyPlugin(regularisation_type, mem_size, alpha=alpha, beta=beta)
        if plugins is None:
            plugins = [plugin]
        else:
